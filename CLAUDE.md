...
10. **Write `docs/decisions.md` entries once per PR, when it's ready to merge — not after every commit.** A design still settling mid-PR produces entries that duplicate or supersede each other before anyone even reviews them. Write the entry once, covering what the PR actually shipped.
