...
9. **Keep documentation concise — verbosity compounds.** Applies to every doc in this repo: Lead with the fact and one line of *why*; point to code or another doc for detail instead of restating it. This matters most for the append-only logs (`decisions.md`, `experiments.md`, `sessions.md`) and `ARCHITECTURE.md`, but it applies everywhere. Also it is better if can write docs when finalize the all the things and it's ready to merge otherwise content can be duplicate or supersede each other — a bloated doc is harder for a human to skim and burns real context/tokens for every agent that reads it.
